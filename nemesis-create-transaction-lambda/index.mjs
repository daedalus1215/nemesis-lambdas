...
export async function handler(event) {
  ...
  try {
    ...
    if (!amount || amount <= 0) {
      return {
        statusCode: 400,
        body: JSON.stringify({ message: "Invalid transaction amount" }),
      };
    }

    if (payload.sub === receiverId) {
      return {
        statusCode: 400,
        body: JSON.stringify({ message: "Cannot send money to yourself" }),
      };
    }
    ...
    if (!sender.Item) {
      return {
        statusCode: 404,
        body: JSON.stringify({ message: "Sender not found" }),
      };
    }
    ...
    if (!receiver.Item) {
      return {
        statusCode: 404,
        body: JSON.stringify({ message: "Receiver not found" }),
      };
    }
    ...
    if (error.code === "TransactionCanceledException") {
      return {
        ...
        body: JSON.stringify({
          message:
            "Transaction failed due to concurrent modification or insufficient balance",
        }),
      };
    }
